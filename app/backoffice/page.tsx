...
import { formatearFechaHoraAR, ventasCerradasPorFecha } from "@/lib/fechas"
...
export default function BackofficePage() {
  ...
  // Valores por defecto vacíos a propósito: los reales se cargan desde la
  // tabla `configuracion` y se editan en Configuración → Cuenta de transferencia.
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        >
          <TabsList className="bg-white border border-gray-200 flex flex-wrap h-auto gap-1 p-1">
            ...
            {/* Ganadores Pasados: ocultado (se reemplazó por el Mural).
          ...
          <TabsContent value="informacion" className="space-y-6">
            {sorteoActual ? (
              <>
                ...
                {/* Cierre de ventas programado */}
                {sorteoActual.estado === "activo" && (
                <Card
                  className={
                    ventasCerradasPorFecha(sorteoActual)
                      ? "border-orange-200 bg-orange-50"
                      : sorteoActual.fecha_cierre_ventas
                ...
                >
                  <CardContent className="p-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        ...
                        <div>
                          ...
                          {ventasCerradasPorFecha(sorteoActual) ? (
                            ...
                            <p className="text-sm text-blue-900">
                              Se puede comprar hasta el{" "}
                              {formatearFechaHoraAR(
                                sorteoActual.fecha_cierre_ventas,
                              )}
                              {sorteoActual.fecha_sorteo_programada &&
                                ` · Sorteo: ${formatearFechaHoraAR(sorteoActual.fecha_sorteo_programada)}`}
                            </p>
                          ) : (
                            <p className="text-sm text-gray-600">
                              Sin programar: se vende hasta llegar al 100% de las
                              chances.
                            </p>
                          )}
                        </div>
                      ...
                      <Button
                        variant={
                          sorteoActual.fecha_cierre_ventas ? "outline" : "default"
                        }
                        size="sm"
                                ...
                                {comprador.instagram_username ? (
                                  ...
                                  <a
                                    href={`https://wa.me/${
                                      ...
                                      `Hola ${comprador.nombre}! Te hablo de ${MARCA}...`,
                                    )}`}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                  >
          ...
          {/* Ganadores Pasados: ocultado (ver TabsTrigger comentado arriba).
